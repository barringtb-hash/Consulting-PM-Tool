 ...
 *
 * Comprehensive social media publishing management interface.
 * Features:
 * - Connect and manage social media platform accounts
 * - Create, schedule, and publish posts across multiple platforms
 * - View scheduled and published posts with status tracking
 * - Quick statistics dashboard
 * - Responsive design with dark mode support
 *
 * @module pages/SocialPublishingPage
 */
...
/**
 * Suggested hashtags for different industries
 */
        ...
        <div>
          ...
          {availablePlatforms.length === 0 ? (
            <p className="text-sm text-neutral-500 dark:text-neutral-400">
              No connected platforms. Please connect platforms in the
              Connections tab.
            </p>
          ) : (
...
/**
 * Reusable error state component for displaying query failures
 */
...
export function SocialPublishingPage(): JSX.Element {
  const { showToast } = useToast();
  ...
  const scheduledPostsQuery = usePosts({ status: 'SCHEDULED', limit: 50 });
  const publishedPostsQuery = usePosts({ status: 'PUBLISHED', limit: 50 });
  ...
  const publishedPosts = publishedPostsQuery.data?.posts || [];
  ...
  }, [syncPlatformsMutation, showToast]);
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900">
      <PageHeader
        ...
        description="Create, schedule, and publish content across your social media platforms"
        icon={Share2}
        actions={
        ...
        >
          ...
          <TabsContent value="posts">
            {allPostsQuery.isLoading ? (
              ...
              <EmptyState
                ...
                title="No posts yet"
                description="Create your first post to start publishing content across your social media platforms."
                action={
                  <Button onClick={() => setShowComposer(true)}>
                    <Plus className="w-4 h-4" />
                    Create Post
          ...
          <TabsContent value="scheduled">
            {scheduledPostsQuery.isLoading ? (
              ...
              <ErrorState
                ...
                message={
                  scheduledPostsQuery.error?.message ||
                  'An error occurred while loading scheduled posts'
                }
                onRetry={() => scheduledPostsQuery.refetch()}
              />
            ) : scheduledPosts.length === 0 ? (
              <EmptyState
                icon={<Clock className="w-8 h-8 text-neutral-400" />}
                title="No scheduled posts"
                description="Schedule posts to automatically publish them at the perfect time."
                action={
                  <Button onClick={() => setShowComposer(true)}>
                    <Calendar className="w-4 h-4" />
                    Schedule a Post
          ...
          <TabsContent value="published">
            {publishedPostsQuery.isLoading ? (
              ...
              <ErrorState
                title="Failed to load published posts"
                message={
                  publishedPostsQuery.error?.message ||
                  'An error occurred while loading published posts'
                }
                onRetry={() => publishedPostsQuery.refetch()}
              />
            ) : publishedPosts.length === 0 ? (
              <EmptyState
                icon={<CheckCircle className="w-8 h-8 text-neutral-400" />}
                title="No published posts"
                description="Posts you publish will appear here with their engagement metrics."
                action={
                  <Button onClick={() => setShowComposer(true)}>
                    <Send className="w-4 h-4" />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                      Social publishing requires API configuration. Please
                      contact your administrator to set up the Ayrshare
                      integration.
                    </p>
                  </div>
                </div>
