    ...
    <Card>
      <div className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="h-4 w-20 bg-neutral-200 dark:bg-neutral-700 animate-pulse rounded" />
        </div>
    ...
    <div className="space-y-3">
      <div className="flex gap-4 pb-2 border-b border-neutral-200 dark:border-neutral-700">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="h-4 w-20 bg-neutral-200 dark:bg-neutral-700 animate-pulse rounded"
          />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex gap-4 py-2">
          {[1, 2, 3, 4].map((j) => (
            <div
              key={j}
              className="h-4 w-20 bg-neutral-100 dark:bg-neutral-800 animate-pulse rounded"
            />
          ))}
        </div>
      ))}
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900">
      <PageHeader
        ...
        description="Track AI tool usage, costs, and performance metrics"
        icon={Brain}
        actions={
            ...
            <>
              ...
              <Card>
                <div className="p-6">
                  ...
                  <div className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
                    {((summary?.data?.successRate || 0) * 100).toFixed(1)}%
                    success rate
                  </div>
                </div>
              </Card>
                ...
                <div className="space-y-4">
                  {(summary?.data?.topTools || []).map((tool, index) => (
                    <div key={tool.toolId} className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900/50 flex items-center justify-center text-primary-600 dark:text-primary-400 font-semibold">
                        {index + 1}
                      </div>
                ...
                <div className="space-y-4">
                  {(summary?.data?.topModels || []).map((model, index) => (
                    <div key={model.model} className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-900/50 flex items-center justify-center text-purple-600 dark:text-purple-400 font-semibold">
                        {index + 1}
                      </div>
                      <div className="flex-1">
                        <div className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
