 ...
 */
router.post('/tenants', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.get('/tenants/current', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
 ...
 */
router.put('/tenants/current', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 * Get tenant branding
 */
router.get('/tenants/current/branding', requireAuth, async (_req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 */
router.put('/tenants/current/branding', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
    ...
    const branding = await tenantService.updateTenantBranding(tenantId, parsed.data);
    res.json({ data: branding });
  } catch (error) {
    throw error;
  }
 ...
 * List tenant domains
 */
router.get('/tenants/current/domains', requireAuth, async (_req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 * Add custom domain
 */
router.post('/tenants/current/domains', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 * Verify domain
 */
router.post('/tenants/current/domains/:domainId/verify', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
 ...
 */
router.delete('/tenants/current/domains/:domainId', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
 ...
 * List tenant users
 */
router.get('/tenants/current/users', requireAuth, async (_req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 * Add user to tenant
 */
router.post('/tenants/current/users', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 * Update user role
 */
router.put('/tenants/current/users/:userId/role', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
    ...
    const tenantUser = await tenantService.updateUserRole(tenantId, userId, parsed.data.role);
 ...
 */
router.delete('/tenants/current/users/:userId', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
    const userId = parseInt(req.params.userId, 10);
 ...
 * List tenant modules
 */
router.get('/tenants/current/modules', requireAuth, async (_req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
 ...
 * Configure module
 */
router.put('/tenants/current/modules/:moduleId', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
    ...
    const module = await tenantService.configureTenantModule(tenantId, parsed.data);
 ...
 * Start module trial
 */
router.post('/tenants/current/modules/:moduleId/trial', requireAuth, async (req: TenantRequest, res: Response) => {
  try {
    const { tenantId } = getTenantContext();
    const trialDays = req.body.trialDays || 14;
