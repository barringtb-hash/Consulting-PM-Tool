 ...
 */
const DELIMITER_PATTERNS: Array<{ pattern: RegExp; replacement: string }> = [
...
/**
 * Result of prompt input validation.
 */
export interface ValidationResult {
  ...
  reason?: string;
  /** The specific pattern that triggered the failure, if any */
  matchedPattern?: string;
}

/**
 * Options for the escapePromptContent function.
 */
export interface EscapeOptions {
  /** Maximum length of the output (default: no limit) */
  maxLength?: number;
  /** Whether to escape JSON special characters (default: true) */
  escapeJson?: boolean;
  /** Whether to neutralize prompt delimiters (default: true) */
...
export function escapeObjectStrings<T extends Record<string, unknown>>(
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (typeof value === 'string') {
      (result as Record<string, unknown>)[key] = escapePromptContent(
        value,
        options,
      );
    } else if (
      ...
      (result as Record<string, unknown>)[key] = escapeObjectStrings(
